...
# this is your 60M‐row filtered panel
ds <- open_dataset(
...
# -------------------------------
# 5.  LOAD PROCESSED READJUSTMENTS
# -------------------------------
...
plan_reajustes_avg <- reaj %>%
  ...
  summarise(percentual_avg = mean(percentual, na.rm = TRUE)) %>%
  collect()
...
# 6b) attach 2020 GDP and propagate to all years
...
# 6c) pull in each muni’s state code (sg_uf) from the Arrow dataset
muni_states <- ds %>%
...
# -------------------------------
# 7.  EXPORT TO STATA (ONLY IN‐SAMPLE MUNICIPALITIES)
# -------------------------------
...
# 9a) Count of municipalities by treatment_status and year (including Out of Sample)
mp %>%
...
# 9b) Missing‐readjustment diagnostics by treatment_status
mp %>%
...
# 9c) Overall readjustment distribution
mp %>%
  summarise(
    obs_total           = n(),
...
# 9e) Distribution of number of plans per municipality
mp %>%
  summarise(
...
# compute average by group
df_plot <- mp %>%
  filter(treatment_status %in% c("Treated", "Control")) %>%
  group_by(year, treatment_status) %>%
  summarise(mean_readj = mean(readjustment, na.rm = TRUE), .groups = "drop")

# pivot wide and compute difference
df_diff <- df_plot %>%
  pivot_wider(names_from = treatment_status, values_from = mean_readj) %>%
...
ggplot(df_diff, aes(x = year, y = diff)) +
  ...
  labs(
    x = "Year",
    y = "Difference in Average Readjustment\n(Treated − Control)",
    title = "Evolution of Readjustment Difference"
  ) +
  theme_classic()
