...
write_dta(
  mp_sample,
  here("data","processed_data","national","dta","municipality_panel_eventdd_EX-SP.dta")
)

# -------------------------------
# 7.  OUTSIDE SP TREATMENT MAP
# -------------------------------
message("Step 8: Plot treatment map for all municipalities except São Paulo (SP)…")

# 1) load all municipalities, drop SP (state code 35)
munis_out_sp <- read_municipality(code_muni = "all", year = 2020) %>%
  filter(code_state != 35) %>%
  mutate(
    cd_municipio = as.numeric(substr(code_muni, 1, nchar(code_muni) - 1))
  )

# 2) bring in your treatment data
map_data <- municipality_treatment %>%
  mutate(cd_municipio = as.numeric(cd_municipio))

# 3) join & recode NAs to "Out of Sample"
map_out_sp <- munis_out_sp %>%
...
# 4) read all state boundaries, drop SP
states_out_sp <- read_state(year = 2020) %>%
  filter(code_state != 35)

# 5) plot with white background
treatment_map_out_sp <- ggplot(map_out_sp) +
  ...
  labs(
    title    = "Treatment Status – Outside São Paulo (Excl. SP)",
...
# 6) save
ggsave(
  here("output","images","national","maps","treatment_status_map_exclSP.png"),
  treatment_map_out_sp,
