...
write_dta(
  mp_sample,
  here("data","processed_data","national","dta","municipality_panel_eventdd_SP.dta")
)

# -------------------------------
# 7.  SP TREATMENT MAP
# -------------------------------
message("Step 8: Plot treatment map for São Paulo (SP)…")

# load only SP municipality geometries (state code 35)
sp_munis <- read_municipality(code_muni = "all", year = 2020) %>%
...
# read only SP state boundary
sp_border <- read_state(code_state = "SP", year = 2020)

# plot with white background
treatment_map_sp <- ggplot(sp_map) +
  ...
  labs(
    title    = "Treatment Status – São Paulo (SP)",
...
# save
ggsave(
  here("output","images","national","maps","treatment_status_map_SP.png"),
  treatment_map_sp,
